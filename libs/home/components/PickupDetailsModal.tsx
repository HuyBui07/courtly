...
import { useJoinPickup } from "../hooks/mutations/useJoinPickup";

export const PickupDetailsModal = () => {
  const { isVisible, details } = usePickupDetailsModalStore();
  const { data: userInfo, isLoading: isLoadingUserInfo } = useGetUserInfo(details.user_id);
  const { data: pickupDetails, isLoading: isLoadingPickupDetails } = useGetPickupDetails(details.court_booking_id);
  const { mutate: joinPickup } = useJoinPickup();

  const isLoading = isLoadingUserInfo || isLoadingPickupDetails;

  const handleJoinPickup = () => {
    joinPickup(details.id);
  };
